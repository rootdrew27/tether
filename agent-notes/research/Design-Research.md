> [!NOTE]
> Pre-MVP exploration. Early survey of comparable systems and design options; written before the MVP spec was committed. Some recommendations made it into MVP (per-tether JSON files under `.tether/tethers/`, UUIDv7 IDs, CLI + Python API split), others did not (free-form type strings — MVP has no link-type field at all; the data model is untyped and semantics live in the required free-form description; `watchdog`-based file watching — MVP has no watcher; markdown-frontmatter secondary surface — not in MVP; content-hash rolling rename detection — MVP uses git's `find-object`). "Already locked in" notes scattered through this doc reflect mid-exploration decisions, not all of which survived. The current design lives in [Tether-Design-MVP](../design/Tether-Design-MVP.md) and the deferred catalog in [Future-Work](../design/Future-Work.md).
