...
- **PreToolUse on additional triggers.** The MVP injects the tether context payload only on `Read`. Other triggers worth exploring as the integration matures:
...
A first piece of this scaffolding now exists in `evals/margin/`: a [Margin](https://github.com/Margin-Lab/evals) harness that runs Claude Code with tether installed and initialized over a SWE-bench-style suite, in a treatment-vs-control split, with a verifier asserting that install, init, and the SessionStart hook fire. It is plumbing only — not yet the seeded HEALTHY/DRIFTED/BROKEN task battery — but it is the substrate the test environment and fragment experiments above would build on.
