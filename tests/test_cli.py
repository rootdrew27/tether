...
def _add_describes(runner: CliRunner) -> str:
...
def test_add_refuses_self_tether(in_project: Path):
    ...
    result = runner.invoke(
        main,
        ["add", "docs/auth.md", "docs/auth.md", "--type", "related"],
    )
    assert result.exit_code == 1
    assert "differ" in result.output


def test_add_refuses_bidirectional_describes(in_project: Path):
    _seed_files(in_project)
    ...
    assert result.exit_code == 1
    assert "cannot be bidirectional" in result.output


def test_status_all_healthy(in_project: Path):
    _seed_files(in_project)
    runner = CliRunner()
    _add_describes(runner)
...
def test_refresh_updates_fingerprint(in_project: Path):
    ...
    assert after.dst.fingerprint != before.dst.fingerprint
...
def test_mv_rewrites_paths(in_project: Path):
    ...
    assert loaded.dst.path == "src/authentication.py"
