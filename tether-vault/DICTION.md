...
| **Artifact**            | One of the two ends of a tether: a path (project-relative POSIX) plus a fingerprint, optionally narrowed to a region by a locator. A whole-file artifact carries no locator. | Knot, endpoint, side, target, node |
| **Region**              | A locator-addressed part of an artifact's file: a Python symbol (`path.py::Class.method`) or a markdown heading path (`path.md::Section/Subsection`). A region artifact drifts only when that part changes; edits elsewhere leave it HEALTHY. | Section, span, fragment |
| **Locator**             | The selector that names a region — `{kind, lang, selector}` (a Python `symbol` or a markdown `heading`). Present only on a region artifact; a whole-file artifact has none. | Selector, anchor |
...
| **tether show**             | List every tether with its description, regardless of state. Structural only — reads records from disk without computing drift or touching git; on a TTY it prints a neutral Rich catalog directly, and emits a plain-text catalog when piped or forced with `--plain`. Takes no tether ID; it lists the whole graph. | List, browse, dump, ls     |
...
- **"Artifact" vs. "tethered file".** The README and casual conversation say "tethered file"; technical writing should use **Artifact**. An artifact is the whole file unless a locator narrows it to a region, so "tethered file" is shorthand only for whole-file artifacts.
