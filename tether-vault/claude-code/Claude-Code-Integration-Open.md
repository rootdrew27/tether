...
- **PreToolUse Read hook for tethered-file context injection.** When the agent reads a tethered file, inject that file's tethers (states, peers, descriptions) as context so the agent knows about the relationship *before* deciding what to edit — not just at Stop. More targeted than the previously-rejected per-edit injection pattern because the trigger filters to tethered files and Read is upstream of action. Depends on the "how tethered is this file" CLI primitive tracked in [[Future-Work]]; both ship together.
